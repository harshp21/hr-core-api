...
describe('Employee API', () => {
  ...
  it('should create employee via API', async () => {
    ...
    expect(response.body.response.email).toBe(`john-${suffix}@example.com`);
  });
  ...
  it('should fetch employee by id', async () => {
    ...
    const id = createRes.body.response.id;

    const res = await request(app).get(`/api/v1/employees/${id}`);

    expect(res.status).toBe(200);
    expect(res.body.response.id).toBe(id);
  });
  ...
  it('should update employee via API with full payload including dateOfJoining', async () => {
    ...
    expect(updateRes.body.response.firstName).toBe('Jane');
    expect(updateRes.body.response.department).toBe('Platform');
  });
  ...
  it('should list employees via API', async () => {
    ...
    const res = await request(app).get('/api/v1/employees?page=1&pageSize=10');
